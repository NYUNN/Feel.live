...
//    bInfoText = true;
    ...
    gui.add(instructionSize.setup("instructionSize", 300, 10, 3000));
    gui.add(effectAStart.setup("effectAStart", 100, 0, 1000));
    gui.add(effectATime.setup ("effectATime", 10,  0, 100));    // maximum 10 second
    gui.add(effectBStart.setup("effectBStart", 300, 0, 1000));
    gui.add(effectBTime.setup ("effectBTime", 30,  0, 100));
    gui.add(effectCStart.setup("effectCStart", 500, 0, 1000));
    gui.add(effectCTime.setup ("effectCTime", 50,  0, 100));
    ...
    // SCENE MANAGER
    sceneManager.setup();
    sceneManager.setPosition(ofGetWindowSize()[0]/2 + 230, 20);
    sceneManager.add(scene0.setup("scene0",  700, 0, 3000 )); // blobs go to attraction
    sceneManager.add(scene1.setup("scene1",  900, 0, 3000 )); // video 1 play
    sceneManager.add(scene2.setup("scene2", 1300, 0, 3000 )); // video 2 play, blobs disappear
    sceneManager.add(scene3.setup("scene3", 1800, 0, 3000 )); // text play
    sceneManager.add(scene4.setup("scene4", 2100, 0, 3000 )); // text falls
...
//                    }
                ...
                // SET LINE TIMING
                if(timer > scene0 && timer <= scene0+30)
                {
                    cout << "line";
                    line = true;
                    ofDrawLine(boids[i][j].location.x, boids[i][j].location.y, boids[i][j].location.x, boids[i][j].location.y);
                } else {
                    line = false;
                }
...
void ofApp::onServerEvent (ofxSocketIOData& data) {
    ...
    if (gotData){
        ...
        v1.setLocation(ofGetWindowSize()[0]*3/8 + ofRandom(ofGetWindowSize()[0]/4), ofGetWindowHeight()/4 + ofRandom(ofGetWindowHeight()/2));
