...
//    gui.loadFromFile("settings.xml");
    ...
    videos[0].load("video/500_Anger.mov");
    videos[1].load("video/500_Anticipation.mov");
    videos[2].load("video/500_Disgust.mov");
    videos[3].load("video/500_Fear.mov");
    videos[4].load("video/500_Joy.mov");
    videos[5].load("video/500_Sadness.mov");
    videos[6].load("video/500_Surprise.mov");
    videos[7].load("video/500_Trust.mov");
    ...
    // VIDEO POSITION
    vidPosX[0] = vidPosX[2] = vidPosX[4] = vidPosX[6] = 0;
    vidPosX[1] = vidPosX[3] = vidPosX[5] = vidPosX[7] = ofGetWindowWidth()/2;
    
    vidPosY[0] = vidPosY[1] = 0;
    vidPosY[2] = vidPosY[3] = ofGetWindowHeight()/4;
    vidPosY[4] = vidPosY[5] = ofGetWindowHeight()*2/4;
    vidPosY[6] = vidPosY[7] = ofGetWindowHeight()*3/4;
...
//        }
    ...
    for (int i = 0; i < 8 ; i++){
        videos[i].draw(vidPosX[i], vidPosY[i], 500, 500);
    }
...
//        ofEnableAlphaBlending();
//        glBlendFunc(GL_SRC_ALPHA, GL_ONE);
//        videos[1].draw(0, 0, ofGetWindowSize()[0], ofGetWindowHeight());
//        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
//        ofDisableAlphaBlending();
//
//        //rippleSize = 20;
...
void ofApp::videoSetup(){
    int nFiles = dir.listDir("video");
    
    if(nFiles) {
        for(int i=0; i<dir.size(); i++) {
            // add the video to the vector
            string filePath = dir.getPath(i);
            videos.push_back(ofAVFoundationPlayer());
            videos.back().load(filePath);
        }
//        videos[0].load("500_Anger.mov");
